-- Create referral system tables (updated version)
...
-- Add referral tracking columns to users table (with proper error handling)
DO $$ 
BEGIN
...
BEGIN
  ...
  IF OLD.onboarding_completed = FALSE AND NEW.onboarding_completed = TRUE AND NEW.referred_by IS NOT NULL THEN
    ...
    BEGIN
      SELECT COALESCE(referral_count, 0) INTO referrer_count 
      FROM users 
      WHERE id = NEW.referred_by;
      ...
      IF referrer_count >= 4 THEN
        ...
        -- Add premium reward (14 days) - only if not already exists
        ...
        -- Add verification boost reward - only if not already exists
        INSERT INTO referral_rewards (user_id, reward_type, reward_value, expires_at)
...
-- Generate referral codes for existing users (safely)
UPDATE users 
SET referral_code = generate_referral_code()
WHERE referral_code IS NULL;

-- Initialize referral_count for existing users
UPDATE users 
